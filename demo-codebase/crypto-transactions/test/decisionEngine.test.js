...
test("denies EU nationalities from trading EU-blacklisted Vietnamese tokens", () => {
  const result = decideCryptoTransaction({
    user: {
      id: "user-123",
      location: "DE",
...
test("denies Vietnamese nationality users from trading AXS", () => {
  const result = decideCryptoTransaction({
    user: {
      id: "user-123",
      location: "VN",
  ...
  assert.equal(result.decision, "DENY");
  assert.equal(result.reasons[0].code, "TOKEN_BLACKLISTED_FOR_NATIONALITY");
  assert.match(result.reasons[0].message, /AXS cannot be traded by users with VN nationality/);
  assert.equal(result.reasons[0].legalBasis.authority, "Demo Vietnam Digital Asset Trading Restriction");
  assert.match(result.reasons[0].legalBasis.citation, /VN-CRYPTO-TRADE-BL-2026-001/);
  assert.match(result.reasons[0].legalBasis.summary, /foreign nationalities remain eligible/);
  assert.equal(result.checks.transactionTokenBlacklist.passed, false);
  ...
  assert.equal(result.checks.transactionTokenBlacklist.originCountry, "VN");
  assert.deepEqual(result.checks.transactionTokenBlacklist.blockedNationalities, ["VN"]);
});

test("permits foreign nationalities from trading AXS", () => {
  const result = decideCryptoTransaction({
    user: {
      id: "user-123",
      location: "VN",

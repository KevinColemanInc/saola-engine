...
Set `transaction.token` to the traded token symbol when the transaction trades a token. AXS is restricted for Vietnamese nationals while foreign nationalities remain eligible, and U2U is noted as a Vietnam-origin token blacklisted for EU nationalities.
...
The AXS restriction is modeled as a demo Vietnam digital asset trading rule: `VN-CRYPTO-TRADE-BL-2026-001, Vietnamese Nationality AXS Trading Ban`. The U2U blacklist is modeled as a demo supervisory ruling: `EU-CRYPTO-TRADE-BL-2026-001, Annex I, Vietnamese-Origin Game and Utility Tokens`. These are intentionally represented as demo law/ruling text rather than verified external token blacklists.
